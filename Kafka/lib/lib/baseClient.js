 ...
 *
 * 获取主题的元数据信息，这包括包含每个zookeeper节点、节点id、主机名和端口的数组。以及包含主题名称、分区、领导号、副本计数和每个分区同步副本的对象。
...
Client.prototype.createTopics = function (topics, isAsync, cb) {
  console.log('[baseClient] createTopics',topics, isAsync, cb);
  ...
  const getTopicsFromKafka = (topics, callback) => {
    this.loadMetadataForTopics(topics, function (error, resp) {
      ...
      console.log('resp[1].metadata ',resp[1].metadata);
