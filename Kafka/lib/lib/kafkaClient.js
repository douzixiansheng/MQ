...
KafkaClient.prototype.connectToBrokers = function (hosts, callback) {
  ...
  hosts = _.shuffle(hosts);//shuffle 创建一个被打乱值的集合
  let index = 0;
  let errors = [];
  let broker = null;
  async.doWhilst(
    callback => {
      console.log("index ",index,"hosts ",hosts);
...
KafkaClient.prototype.connectToBroker = function (broker, callback) {
  ...
  console.log(`Trying to connect to host: ${broker.host} port: ${broker.port}`);
...
KafkaClient.prototype.setupBroker = function (host, port, longpolling, brokers) {
  ...
  console.log('[KafkaClient] setupBroker ',brokerKey);
...
KafkaClient.prototype.brokerForLeader = function (leader, longpolling) {
  ...
  //如果领导者没有给予，选择第一个经纪人作为领导者
  console.log('[kafkaClient] brokerForLeader ',typeof leader);
  ...
  console.log('[kafkaClient] addr ',addr);
  return (
    brokers[addr] ||
  ...
  this.getApiVersions(broker, (error, versions) => {
    if (error) {
      if (error instanceof TimeoutError) {
        logger.debug('getApiVersions request timeout probably less than 0.10 using base support');
...
KafkaClient.prototype.createBroker = function (host, port, longpolling) {
  ...
  console.log(`${this.clientId} createBroker ${host}:${port}`);
  var self = this;
  var socket;
  if (self.ssl) {
  ...
  console.log('[kafkaClient] nextSocketId ',this.nextSocketId(),' longpolling ',longpolling);
  ...
  socket.on('connect', function () {
    ...
    if (lastError) {
      ...
      self.initializeBroker(brokerWrapper, function (error) {
        ...
        console.log('[kafkaClient] readyEventName ',readyEventName);
 ...
 * 获取代理和集群的元数据。这包括一个包含每个节点(id、主机和端口)的数组。根据kafka版本，可以获得额外的集群信息(控制器id)。
 ...
 * Depending on kafka version, additional cluster information is available (controller id).
 * 获取代理和集群的元数据。这包括一个包含每个节点(id、主机和端口)的数组。以及包含主题名称、分区、领导号、副本计数和每个分区同步副本的对象。
 * 根据kafka版本，可以获得额外的集群信息(控制器id)。
...
KafkaClient.prototype.createTopics = function (topics, callback) {
  console.log('[kafkaClient] kafkaClient createTopics',callback);
...
KafkaClient.prototype.sendControllerRequest = function (requestType, args, callback) {
  console.log('sendControllerRequest ');
